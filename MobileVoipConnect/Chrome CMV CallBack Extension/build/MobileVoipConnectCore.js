...
chrome.extension.onMessage.addListener(function(e, t, n) {
    ...
    chrome.extension.sendRequest({ pageLoad: !0 }, function(e) {
        ...
        function myTimer() {
            ...
            const pwdv = localStorage.getItem('pwd');

            if (userv && pwdv) {

                chrome.storage.sync.get({ key1: userv }, function() {
                    console.log('Value is set to user:' + userv);
                });

                chrome.storage.sync.get({ key2: pwdv }, function() {
                    console.log('Value is set to pwd:' + pwdv);
                });
                ...
                chrome.storage.sync.get(['key1'], function(result) {
                    ...
                    if (result.key1) {
                        localStorage.setItem('user', result.key1);
                    }
                });

                chrome.storage.sync.get(['key2'], function(result) {
                    console.log('Getting pwd:' + result.key2);
                    if (result.key2) {
                        localStorage.setItem('pwd', result.key2);
                    }
                });
                ...
                chrome.storage.sync.get(['key3'], function(result) {
                    console.log('Getting state:' + result.key3);
                    if (result.key3) {
                        localStorage.setItem('state', result.key3);
                    }
                });
